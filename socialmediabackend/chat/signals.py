...
def send_other_notification(sender, instance, created, **kwargs):
    if created:
        channel_layer = get_channel_layer()
        notifications = Notification.objects.filter(is_seen=False, user=instance.user)
        user_id = str(instance.user.id)
        data = {
            "notifications": [
...
def send_onlineStatus(sender, instance, created, **kwargs):
    if not created:
        ...
        async_to_sync(channel_layer.group_send)(
            "online_users", {"type": "send_onlineStatus", "value": json.dumps(data)}
        )
